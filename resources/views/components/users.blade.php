        ...
        @foreach ($users as $user)
            <tr>
                ...
                <td>
                    ...
                    @endif
                    @if(Auth::user()->role == 'moderator' && request('tab', 'active') == 'active' && $user->role != 'admin')
                        ...
                        <div class="collapse multi-collapse-ban-{{ $user->id }}" id="timeInput">
                            <form class="input-group input-group-sm" method="POST" action="{{ route('user.ban', ['id' => $user->id]) }}">
                                @csrf
                                @method('PUT')
                                <input type="datetime-local" class="form-control" value="{{ $user->bannedTill }}" min="{{ now()->setTimezone('UTC')->format('Y-m-d\TH:i') }}" name="bannedTill" aria-label="Select datetime" aria-describedby="button-addon1" required>
